# -*- coding: utf-8 -*-
"""
Created on Tue Aug  2 16:30:52 2022
...
def GA(func, n_dim, size_pop, max_iter, lb, ub):
    ...
    bestIndividual = numpy.zeros(n_dim)
    scores = numpy.random.uniform(0.0, 1.0, size_pop)
    bestScore = float("inf")

    ga = numpy.random.uniform(0, 1, size=(size_pop, n_dim)) * (ub-lb) + lb
    ...
    for l in range(max_iter):
        ...
        # crossover
        ga = crossoverPopulaton(ga, scores, size_pop, cp, keep)

        # mutation
        mutatePopulaton(ga, size_pop, mp, keep, lb, ub)

        ga = clearDups(ga, lb, ub)

        scores = calculateCost(func, ga, size_pop, lb, ub)
